...
describe("plan adapter star expansion", () => {
	...
	it("preserves lateral output columns as derived outputs", () => {
		...
		const plan = buildPlanFacts(session.doc.statements[0]!, sql, {
			dialect: "databricks",
			schema: { columnsFor: (parts: string[]) => parts.join(".").toLowerCase() === "demo.base" ? [{ name: "id" }] : undefined },
	...
	it("propagates physical inputs through a lateral-derived subquery output", () => {
		const sql = "SELECT x.busi_date FROM (SELECT date_add(t.dt, y.pos) AS busi_date FROM demo.base t LATERAL VIEW posexplode(array(1)) y AS pos, val) x";
		const session = SqlSession.create(sql, "databricks");
		const plan = buildPlanFacts(session.doc.statements[0]!, sql, {
			dialect: "databricks",
			schema: { columnsFor: (parts: string[]) => parts.join(".").toLowerCase() === "demo.base" ? [{ name: "dt" }] : undefined },
			include_expression_dependencies: true,
		});
		...
		expect(rootProject.expressions[0]?.input_columns).toMatchObject([
			{ name: "busi_date", qualifier: "x", resolution: "PHYSICAL", physical: [{ table: "demo.base", column: "dt" }] },
		]);
	});

	it("propagates physical inputs through a computed lateral-derived output", () => {
		const sql = "SELECT c_sp.busi_date FROM (SELECT date_add(strt_date, pos) AS busi_date FROM (SELECT start_date AS strt_date FROM demo.source) x LATERAL VIEW posexplode(array(1)) y AS pos, val) c_sp";
		const session = SqlSession.create(sql, "databricks");
		const plan = buildPlanFacts(session.doc.statements[0]!, sql, {
			dialect: "databricks",
			schema: { columnsFor: (parts: string[]) => parts.join(".").toLowerCase() === "demo.source" ? [{ name: "start_date" }] : undefined },
			include_expression_dependencies: true,
		});
		...
		expect(plan.unknowns).toEqual([]);
		expect(rootProject.expressions[0]?.input_columns).toMatchObject([
			{ name: "busi_date", qualifier: "c_sp", resolution: "PHYSICAL", physical: [{ table: "demo.source", column: "start_date" }] },
		]);
	});
	...
	it("resolves unqualified outputs from a set-operation derived table", () => {
		const sql = "SELECT rec_id FROM (SELECT concat('a', id) AS rec_id FROM demo.a UNION ALL SELECT concat('b', id) AS rec_id FROM demo.b) casttable";
		const session = SqlSession.create(sql, "databricks");
		const schema = { columnsFor: (parts: string[]) => {
		...
		expect(rootProject.expressions[0]?.input_columns?.[0]).toMatchObject({
			name: "rec_id",
			resolution: "PHYSICAL",
			physical: [{ table: "demo.a", column: "id" }, { table: "demo.b", column: "id" }],
		});
	});

	it("keeps simple-case inputs and inputless computed outputs across set operations", () => {
		const sql = "SELECT label, generated FROM (SELECT CASE kind WHEN 'a' THEN 'A' ELSE 'B' END AS label, from_unixtime(unix_timestamp()) AS generated FROM demo.a UNION ALL SELECT CASE kind WHEN 'b' THEN 'B' ELSE 'A' END AS label, from_unixtime(unix_timestamp()) AS generated FROM demo.b) x";
		const session = SqlSession.create(sql, "databricks");
		const schema = { columnsFor: (parts: string[]) => {
			const table = parts.join(".").toLowerCase();
		...
		if (rootProject?.type !== "project") throw new Error("root project relation missing");
		expect(rootProject.expressions[0]?.input_columns).toMatchObject([
			{ name: "label", resolution: "PHYSICAL", physical: [{ table: "demo.a", column: "kind" }, { table: "demo.b", column: "kind" }] },
		]);
	...
	it("keeps mixed physical and SQL-candidate branches across a set operation", () => {
		...
		const plan = buildPlanFacts(session.doc.statements[0]!, sql, {
			dialect: "databricks",
			schema: { columnsFor: (parts: string[]) => parts.join(".").toLowerCase() === "demo.physical" ? [{ name: "book" }] : undefined },
			include_expression_dependencies: true,
		});
