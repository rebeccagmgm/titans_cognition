...
// compensating churn. Regenerate deliberately: SQL_STATIC_LINEAGE_UPDATE_BASELINE=1.
...
describe.skipIf(!existsSync(ROOT))("databricks vs Spark's own analyzer schemas (sql-tests goldens)", () => {
	it("grades analyze() output types against the vendored v4.2.0 goldens", () => {
		...
		if (process.env.SQL_STATIC_LINEAGE_UPDATE_BASELINE) {
			writeFileSync(BASELINE, `${lines.join("\n")}\n`);
		} else {
			expect(existsSync(BASELINE), "baseline missing — regenerate with SQL_STATIC_LINEAGE_UPDATE_BASELINE=1").toBe(true);
