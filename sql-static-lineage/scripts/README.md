...
These scripts are case-specific helpers around the sql-static-lineage library. Run them
from the `sql-static-lineage/` repository root so their evidence and output paths resolve
consistently.
...
The scripts consume local evidence snapshots and do not alter the sql-static-lineage
library under `src/`.
