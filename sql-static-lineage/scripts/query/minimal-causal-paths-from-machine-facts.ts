...
import { loadMachineFactsGraphInputs } from "./machine-facts-graph-projection.ts";

const workspace = resolve(import.meta.dirname, "../../..");
const PROJECTION_VERSION = "1.0.1";
...
function inputResolutionStatus(
	expression: JsonRecord | undefined,
): "RESOLVED" | "PARTIAL" | "UNRESOLVED" | "DERIVED_OUTPUT" | "NO_PHYSICAL_INPUT" {
...
function resolutionGap(expression: JsonRecord | undefined, label: string): string | undefined {
	...
	return status === "PARTIAL" || status === "UNRESOLVED"
		? `${label} input dependency status is ${status}`
		: undefined;
}
...
function runCli(): void {
	...
	const factsRoot = resolve(workspace, process.argv[3] ?? "machine-facts");
	const profile = JSON.parse(readFileSync(profilePath, "utf8")) as { case_id?: string };
	const caseId = profile.case_id ?? basename(profilePath, ".json");
	const outputDir = resolve(factsRoot, "projections", "minimal-causal-paths", caseId);
	const outputPath = join(outputDir, "minimal-causal-paths.json");
	const manifestPath = join(outputDir, "projection-manifest.json");
	...
	const taskManifests = inputs.profile.tasks.map((task) => {
	...
	const projectionManifest = {
		...
		source_contract_version: "machine-facts-v1",
		assembler_version: "minimal-causal-paths-v1",
		projection_type: "minimal-causal-paths",
		case_id: caseId,
	...
	writeFileSync(outputPath, resultBytes, "utf8");
	writeFileSync(manifestPath, `${JSON.stringify(projectionManifest, null, 2)}\n`, "utf8");
	console.log(
		JSON.stringify(
			{
				output: outputPath,
