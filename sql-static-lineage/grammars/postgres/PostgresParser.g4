/*
 * ANTLR4 parser grammar for PostgreSQL (split lexer + parser pair).
 *
 ...
 * the Go superClass PostgreSQLParserBase was dropped — its only grammar-reachable hook,
 * ParseRoutineBody (CREATE FUNCTION body re-parsing), is out of scope here, the same call the
 * Redshift fork dropped. The upstream
...
mergestmt
   : with_clause? MERGE INTO ONLY? qualified_name STAR? alias_clause? USING (select_with_parens | ONLY? qualified_name) alias_clause? ON a_expr
...
merge_insert
   : INSERT (OPEN_PAREN insert_column_list CLOSE_PAREN)? (OVERRIDING (SYSTEM_P | USER) VALUE_P)? (values_clause | DEFAULT VALUES)
   ;
...
common_table_expr
   : name opt_name_list? AS opt_materialized? OPEN_PAREN preparablestmt CLOSE_PAREN search_clause? cycle_clause?
   ;
