...
**固定日期**: 2026-08-15
**用途**: Logical Plan Facts adapter 的回归基准 (SQL → sqllens → plan-facts-118141.json)
...
**契约**: v1.1 (meta.contract_version=1.1.0, adapter=0.2.0, parser=sqllens 1.8.0)
...
8. **条件列物理解析** (给 schema 后): 57/60 列追到基表:
   ...
   - 3 列未解析 (c_sp/c_ba/cc 的 `busi_date`, sqllens followColumn 对 lateral
...
- 条件列物理解析对 lateral 子查询别名列 (c_sp/c_ba/cc 的 busi_date) 为 null ——
  sqllens followColumn 盲区, unknowns 已显式记录
