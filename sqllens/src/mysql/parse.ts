...
/** The CST rooted at `root` (a semicolon-separated batch of statements); see
		...
		return Object.defineProperty(base as ParseResult, "tokens", {
			get: () =>
				(cached ??= mapTokens(
					...
					tokens.getTokens(),
					"mysql",
					deriveConsumedAs((base as ParseResult).tree, CONSUMED_AS_RULES.mysql),
				)),
			enumerable: true,
			configurable: true,
		});
...
function attachErrorCounter(lexer: Lexer, parser: MysqlParser, listener: ANTLRErrorListener): void {
