/*
 * ANTLR4 parser grammar for Amazon Redshift (split lexer + parser pair).
 *
 * Forked from bytebase/parser, path redshift/ (see RedshiftLexer.g4 for provenance + license).
 *   upstream:  https://github.com/bytebase/parser  (redshift/)  commit 57b6ef7a  retrieved 2026-06-14
 *
 * Local edits for the antlr4ng TypeScript target: superClass = RedshiftParserBase was dropped and
 * its only grammar-referenced method, ParseRoutineBody (which re-parsed CREATE FUNCTION bodies),
 * was removed — CREATE FUNCTION bodies are out-of-scope object DDL and nothing reads the re-parse.
 */
...
createfunc_opt_list
   : createfunc_opt_item+
   {
                /* routine-body re-parse omitted: CREATE FUNCTION bodies are out-of-scope object DDL */
            }
