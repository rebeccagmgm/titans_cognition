...
export function validateBundle(bundleDir: string): string[] {
	...
	try {
		const schemaPath = join(workspace, "sqllens", "schemas", "machine-facts.schema.json");
	...
	try {
		recordSchemas = (json<JsonRecord>(join(workspace, "sqllens", "schemas", "machine-facts-records.schema.json")).properties ?? {}) as JsonRecord;
	} catch (error) {
...
function buildTaskBundle(
	...
	const manifest: MachineFactsManifest = {
		...
		method: {
			dialect: profile.dialect,
			parser: { engine: "sqllens", version: parserVersion },
...
export function rebuildIndex(root: string): ProfileRunResult["index"] {
	...
	try {
		indexSchema = (json<JsonRecord>(join(workspace, "sqllens", "schemas", "machine-facts-records.schema.json")).properties as JsonRecord)["task-fact-index.jsonl"] as JsonRecord;
	} catch (error) {
