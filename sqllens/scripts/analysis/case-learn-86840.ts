...
stage(3, "ir", "IR / 关系结构", planStatus, {
	method: "sqllens QueryExpr IR plus plan-adapter relation outline",
	statements: plans.map((plan, statementIndex) => ({
		statement_index: statementIndex,
		sqllens_ir: serializeIr(session.doc.statements[statementIndex].ast),
...
const guide =
	[
		...
		"- 07：plan facts 比 sqllens 原始结果多保存了哪些事实？",
