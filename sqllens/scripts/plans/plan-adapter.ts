// ============================================================================
// plan-adapter —— 薄 adapter: sqllens scope 树 → Logical Plan Facts (JSON)
//
// 不做任何解析: 原料全部来自 sqllens 已建模的 IR + scope:
...
/** 直接遍历 sqllens IR，保留连接/过滤/表达式判断所需的结构事实。 */
...
/** 物理解析: 用 sqllens lineageAt 把列引用追到基表 (喂 schema 后)。
...
export interface PlanAdapterOptions {
	...
	/** 可选 schema (sqllens Schema 实例), 提供后条件列 physical 解析启用。 */
	schema?: unknown;
...
export function buildPlanFacts(
	...
	function buildScope(scope: Scope, path: string): string {
		...
		if (body.kind === "setop") {
			...
			if (branchIds.length === 0) {
				unknowns.push({ node_id: id, field: "branches", reason: "setop 无 branches (sqllens 未建模分支)" });
			}
	...
	// parser 版本 (sqllens package.json)
