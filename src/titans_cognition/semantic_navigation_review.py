...
QUALIFIER_LABELS = {
    ...
    "currency_basis": "币种基准",
    "measure_basis": "计量口径",
    "aggregation_state": "聚合状态",
    "availability_state": "可用状态",
    "cashflow_direction": "现金流方向",
...
def build_semantic_navigation_review(
    ...
    (review_root / "index.html").write_text(_review_html(), encoding="utf-8")
...
<script src="data/projection.js"></script><script>
const P=window.SEMANTIC_NAV_PROJECTION,C=window.SEMANTIC_NAV_CATALOG,S=window.SEMANTIC_NAV_SHARDS||{};let stage=P.stages.find(x=>x.id==='contract-lifecycle')?.id||P.stages[0].id,concept=null,expr=null,filters=new Set();const $=id=>document.getElementById(id),esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function loadConcept(id){concept=id;expr=null;filters.clear();renderMap();if(S[id])return showConcept(S[id]);const row=C.find(x=>x.id===id);const s=document.createElement('script');s.src=`data/concepts/${row.shard}`;s.onload=()=>showConcept(S[id]);document.body.appendChild(s)}
function showConcept(d){const c=d.concept,qs=[...new Map(d.expressions.flatMap(e=>e.qualifiers).map(q=>[q.dimension+q.value,q])).values()];const stageName=id=>P.stages.find(x=>x.id===id)?.label||id;const roleName={CORE_OBJECT:'核心对象',BUSINESS_EVENT:'业务事件',CROSS_STAGE:'跨环节关联'};$('index').innerHTML=`<h3>当前概念：${esc(c.label)}</h3><div class="muted">候选定义</div><p>${esc(c.definition)}</p><div class="chips"><span class="tag">${esc(c.areaLabels.join(' / ')||'导航待挂接')}</span><span class="tag">${esc(c.supportStatus)}</span></div><div class="boundary">来源概念：${esc(c.sourceLabels.join('、'))}</div><div class="group-title">生命周期入口</div>${c.lifecycleEntries.map(x=>`<div class="physical"><div><span class="tag">CONFIGURATION_SEED｜非证据</span> ${esc(stageName(x.stage_id))} · ${esc(roleName[x.role]||x.role)}</div><div class="boundary">${esc(x.seed_reason)}；尚无独立 Evidence ID 证明该阶段挂接。</div></div>`).join('')}<div class="section"><div class="group-title">属性表达筛选</div><div id="filters" class="chips"><button class="chip active" data-filter="">全部</button>${qs.map(q=>`<button class="chip" data-filter="${esc(q.dimension+':'+q.value)}">${esc(q.dimensionLabel)}：${esc(q.valueLabel)}</button>`).join('')}</div></div><div class="group-title">属性表达 <span class="count">字段 / 表</span></div><div id="expressions"></div>`;$('filters').querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{filters.clear();if(b.dataset.filter)filters.add(b.dataset.filter);$('filters').querySelectorAll('.chip').forEach(x=>x.classList.toggle('active',x===b));renderExpressions(d)});renderExpressions(d)}
function renderExpressions(d){const rows=d.expressions.filter(e=>!filters.size||e.qualifiers.some(q=>filters.has(q.dimension+':'+q.value)));$('expressions').innerHTML=rows.map(e=>`<div class="expr ${expr===e.id?'active':''}" data-expr="${e.id}"><strong>${esc(e.label)}</strong><span class="muted">${e.fieldCount} / ${e.tableCount}</span> ${e.qualifiers.map(q=>`<span class="tag">${esc(q.valueLabel)}</span>`).join('')}</div>`).join('')||'<div class="empty">没有匹配表达</div>';$('expressions').querySelectorAll('[data-expr]').forEach(b=>b.onclick=()=>{expr=b.dataset.expr;renderExpressions(d);showExpression(d.expressions.find(x=>x.id===expr),d.concept)})}
function showExpression(e,c){const status=e.supportStatus==='SUPPORTED'?'status':'status provisional';$('detail').innerHTML=`<div class="detail"><h3>当前表达：${esc(e.label)}</h3><p>基础概念：<strong>${esc(c.label)}</strong></p><p>证据状态：<span class="${status}">${esc(e.supportStatus)}</span></p><h3>限定条件</h3>${e.qualifiers.length?e.qualifiers.map(q=>`<div>├─ ${esc(q.dimensionLabel)}：${esc(q.valueLabel)}</div>`).join(''):'<div class="muted">未识别明确限定</div>'}<h3>业务上下文</h3>${e.contexts.length?e.contexts.map(x=>`<div>├─ ${esc(x.label)} <span class="muted">${esc(x.type)}</span></div>`).join(''):'<div class="muted">当前无独立上下文证据</div>'}<h3>物理实现</h3>${e.physical.slice(0,120).map(x=>`<div class="physical"><div>${esc(x.schema)} · ${esc(x.table)}</div><div class="field">└─ ${esc(x.column)}</div><div>${esc(x.comment||'无中文注释')}</div><div class="boundary">${esc(x.columnId)}</div></div>`).join('')||'<div class="muted">没有物理字段</div>'}<h3>逐项证据</h3>${e.evidence.length?e.evidence.slice(0,120).map(x=>`<div class="physical"><div>${esc(x.role)} · ${esc(x.type)} · ${esc(x.label)}</div><div class="boundary">${esc(x.id)} · ${esc(x.sourceRef)}</div></div>`).join(''):'<div class="muted">当前表达没有独立 Evidence 引用</div>'}<details><summary>断言与方法（${e.assertions.length}）</summary>${e.assertions.map(x=>`<div class="physical"><div>${esc(x.predicate)} · ${esc(x.status)}</div><div class="boundary">${esc(x.id)} · ${esc(x.methodId)} · ${esc(x.objectId)}</div></div>`).join('')}</details><h3>证据边界</h3>${e.conflicts.length?'<div>△ 存在证据冲突</div>':''}<div>△ 正式业务定义待确认</div></div>`}
