...
TRADEFLOW 字段语义 V2 使用独立候选 Projection：首屏只加载轻量字段族、概念与表目录，字段搜索目录在首次搜索或打开待判断/冲突时加载，概念/表明细按哈希分片加载，所有结果列表分页。字段族是宽发现入口，其详情必须列出互不等价的具体复合概念；直接表达字段与仅相关字段必须分开展示。业务页面使用中文“字段族、业务概念、相关表达、限定条件、直接字段、相关字段”，原始 relation/Facet 枚举只放在折叠技术详情。页面支持概念、字段、注释、表名搜索、限定条件组合筛选，以及概念—字段—表双向反查和 Panorama Object Card/V1 链接。完整页面建设受 semantic-shape Gate 约束，Gate 未通过时只保留 Canonical 文件及 JSON/Markdown 比较报告。V2 页面不得读取表级业务分类改变字段语义，只能通过稳定 `column_id/asset_id` 保留未来组合入口。
...
表语义页面只在五组调查卡的信息模型 Gate 通过后生成。页面提供业务协作组、物理变体、结构邻域、Conflict 和 Unknown 的独立入口；同一表在多个入口始终复用一个 `asset_id` 和画像。首屏使用有界目录，详情按分片加载并分页；字段辅助、结构传播提示和方法分数必须与直接候选分层显示，方法分数不得显示为正确概率。

配置中的固定旅程只生成调查卡，不等于业务协作组。业务协作组入口只有在每个成员具有非配置职责证据且组内证据关系图连通时才能出现；种子规范候选与语料发现职责表达必须分开展示。Gate 失败时只保留类型化结果、失败调查卡和诊断，不复用先前运行的审阅页面。

页面可浏览只属于工程结果。固定 TRS、期权和名称反例旅程获得用户审阅前，`reader_delivery` 保持未交付；局部旅程被接受也不自动接受全部表标签、物理变体或方法。
