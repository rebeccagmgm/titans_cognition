...
- `field_semantic_results`：每个 `column_id` 一条任务结果，包含 `outcome=SINGLE_CANDIDATE/COMPETING/UNKNOWN`、零到多个嵌套候选绑定、方法局部分数、支持/冲突 Source Ref、解释和限制；Unknown 不引用伪造概念。
...
页面首屏只加载概念/Facet摘要，详情通过分片 JSON 或 Worker 按需读取；任何列表分页。Object Card 链接复用现有 URI 规则。本 Change 不把表级业务主题接入审阅页；稳定物理 ID 仅为未来独立组合 Projection 保留连接点。
...
5. 执行语义形态 Gate；若结构硬门或结果级双向查询未通过，保留失败结果并停止完整页面建设。
